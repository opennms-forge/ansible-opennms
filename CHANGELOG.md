...
- **`make check-render` covers `stub_mimir`** with a single-node and a cluster fixture as a pair. They pin the dicts the wrapper hands to the fork rather than the file the fork renders from them, since that file is the fork's contract and the fork's test (#186).
