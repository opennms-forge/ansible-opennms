...
### Changed
- **`stub_mimir` delegates the Mimir lifecycle to `indigo423.grafana.mimir`.** The collection already depended on that fork for Grafana, and the fork's `mimir` role does the same install, config render, restart and readiness wait this role carried its own copy of. What stays here is what the fork's generic role does not know: the systemd start-limit drop-in laid down before the package, the explicit address in every ring and the query-frontend, the cluster shape derived from `mimir_cluster_group`, multitenancy off, the OpenNMS limits, and the two assertions. The rendered `/etc/mimir/config.yml` carries every setting it did before, in the fork's section order, plus the paths and URLs the fork always writes. Needs `indigo423.grafana` 7.1.0, which added the top-level passthrough and the readiness knobs this wrapper uses. The drop-in's ordering argument is unchanged and was re-verified across a package upgrade (#TBD).
- **Grafana Mimir 3.0.4 to 3.2.1**, inherited from the fork's pin. The collection no longer declares `mimir_version`; set it in inventory to pin a different one, the name is the same in both roles (#TBD).
- **`make check-render` covers `stub_mimir`** with a single-node and a cluster fixture as a pair. They pin the dicts the wrapper hands to the fork rather than the file the fork renders from them, since that file is the fork's contract and the fork's test (#TBD).

### Removed
- **Breaking: `mimir_http_port`, `mimir_data_dir`, `mimir_arch` and `mimir_pkg_url` are gone from `stub_mimir`.** The first two are renamed to the fork's `mimir_http_listen_port` and `mimir_working_path`, same values. The architecture is derived from facts. A mirror sets the fork's `mimir_download_url_deb` directly. Setting any of the four fails the run naming the replacement, as #182 did for the pinning variables (#TBD).
...
### Added
- **The pinning variables are declared in `meta/argument_specs.yml`.** All three installing roles now declare their package list, `opennms_pin_origin` and `opennms_unattended_upgrade_blacklist` with types and descriptions, so `ansible-doc -t role` shows the interface and a value of the wrong type fails before any task runs. `opennms_minion` gains its first `argument_specs.yml`. Note that validation rejects an unconvertible value but does not write coerced values back into play scope, so `| bool` is still load-bearing in the tasks (#170).
- **The preferences file and the unattended blacklist are covered by `make check-render`.** Three fixtures, one per installing role, recording both files. They exist as a set: the point is that the roles render different content from the same templates, so covering one would pass while the values reaching the other two were wrong (#170).
- **`stub_pyroscope` deploys a Grafana Pyroscope server to receive the profiles.** The collection configured Core, Minion and Sentinel to push continuous profiles and could not deploy anything to push them to. In scope by the collection's own rule, since OpenNMS has a wire to it, and a stub by the same standard as the others: local disk, no object store, no retention, no auth. Three things needed thought rather than copying — the managed config writes only the settings the role owns rather than the 28 KB of commented defaults the package ships, the data location is the unit's `WorkingDirectory` because v2 resolves five paths relative to it, and the role waits on `/ready` because Pyroscope serves nothing for about 90 seconds of a cold start while its components report ready in series (#177).
- **A pinned Pyroscope Java agent for all three components.** The collection shipped no agent of its own: Core got one from its package, but Horizon 36 builds against 0.13.1, and Minion and Sentinel got none at all. A `pyroscope_agent` role now installs a checksummed pyroscope-java 2.9.2 jar, and each component role wires it in — Core over the packaged jar at the path `bin/opennms` hardcodes, Minion and Sentinel through `EXTRA_JAVA_OPTS` rather than `JAVA_OPTS`, which is the knob the pristine file documents for heap. Core's install is unconditional and inert until `PYROSCOPE_AGENT_ENABLED` is set; Minion and Sentinel have no launcher toggle, so a populated `opennms_minion_pyroscope` or `opennms_sentinel_pyroscope` dict is the toggle and an empty one downloads nothing (#173).

### Changed
- **Breaking: `opennms_pinned_packages` is now named per role.** Use `opennms_core_pinned_packages`, `opennms_minion_pinned_packages` or `opennms_sentinel_pinned_packages`. All three roles held a different list under one name, so a single `group_vars` entry replaced every role's list: a Minion host then pinned Core's four packages, none of which it installs, while `opennms-minion` got no entry at all and sat unpinned at priority 500. Measured on a colocated host, where `apt-cache policy` reported `opennms-common` at 1001 and `opennms-minion` at 500. Setting the old name now fails the run with a message naming the replacement (#170).
- **Breaking: the two APT file paths are no longer settable from inventory.** `opennms_apt_preferences_file` and `opennms_unattended_blacklist_file` are gone from `defaults/`; each role derives its own from an `opennms_component` constant in `vars/`. Under one shared name, a single override pointed all three roles at one file and the last to run decided its contents, which is exactly the collision the per-role naming exists to prevent. Setting either now fails the run. A per-role path is the mechanism, not a preference, so there is no replacement variable (#170).
- **The per-role guarantee now holds for the variables, not only for the defaults.** #169 corrected the READMEs to say the guarantee applied to the defaults, which was accurate but described a shape that could still be broken from inventory. It cannot be now (#170).
- **A host carries one OpenNMS major, and the collection now says so.** When a second role in the same run asks for a different major, the run fails naming both roles and both majors, instead of the last role to run silently deciding the major for the whole host. The packaging does not prevent the mixed case — `opennms-minion` and `opennms-sentinel` depend on nothing from OpenNMS — but all three roles write one sources file, so the others' pins would stop matching anything fetchable. The check compares against a fact recorded during the run, so a deliberate major upgrade needs no exemption, and it is per host, so a distributed topology never triggers it (#170).
- **The APT source line is covered by `make check-render`.** It is written from a template rather than inline content so the harness can reach it, and two fixtures record it: one at a major the role's fallback does not name, which is the only kind that can detect this class of defect, and one with no caller version at all, which pins the fallback path (#170).
- **`make check-render` compares rendered configuration to recorded expectations.** Renders role templates against fixture inventories and asserts the output byte for byte, with no hosts, no services and no network. It exists for the class of defect `ansible-lint` cannot see, because the YAML is valid either way: two shipped in one afternoon — values written unquoted into a file that systemd parses *and* a shell sources, then a fix for that which emitted doubled backslashes. Five fixtures cover the escaping-sensitive `opennms.conf`, the Minion and Sentinel Pyroscope stanzas that both defects shipped in, the one existing inventory-derived address (three brokers, joined and sorted), and the single-node versus cluster shapes of `stub_kafka` as a pair, since either alone would pass while the shape logic was broken. Re-record an intended change with `make check-render-record`, which is a separate target so the new expectation appears in the diff. Runs in CI as the `render-config` check.
- **The Pyroscope environment stanza is rendered from a template file.** `opennms_minion` and `opennms_sentinel` previously built it inline in the `blockinfile` task, which put the escaping — the part that had already broken twice — somewhere no check could reach. The body now lives in each role's `templates/pyroscope-env.j2` and is applied with `lookup('template', ...)`. Rendered output is byte-identical, verified against the previous inline form before the roles were touched, so no deployed file changes and no variable moves.
...
- **Breaking: `opennms_jvm_conf` merges over the role's JVM defaults instead of replacing them.** `opennms_env` already merged over its defaults; this dict did not, two lines away in the same template, so tuning the heap silently dropped the profiling configuration and turning profiling on silently dropped anything else. The `group_vars` example needed a warning saying so, which is the wrong shape for a knob. Setting `PYROSCOPE_AGENT_ENABLED: 1` alone is now enough. **An inventory that set the dict in order to omit the three `PYROSCOPE_*` keys will see them rendered again**, at their inert defaults, and the duplicate-key guard now compares against the merged dict, so a `PYROSCOPE_*` default restated in `opennms_env` aborts the run naming the key. Scoped deliberately to this one dict; issue #4 covers the same wart elsewhere, where merging would change what a half-filled dict means (#176).
- **`inventory/opennms-stack.yml` is a genuinely distributed reference topology.** It declared eight host entries under four cosmetic names that all resolved to one address, and `inventory/group_vars/opennms_stack/vars.yml` shipped the values that would fix that commented out. Database, broker, Core and Minion now sit on distinct hosts with every endpoint and reachability value filled in, each derived value carrying the literal alternative commented beneath it. The colocated arrangement moves to `inventory/simple-stack.yml`, labelled a test rig with a note that a colocated Minion has no segment to isolate. Endpoints are declared in the inventory rather than derived in the roles, because past the POC case a service endpoint is a VIP, a pooler or a DNS name and not a host in the inventory; cluster shape stays derived from group membership, because there the group is the answer (#180).
- **OpenNMS Horizon 36.0.3 to 36.0.4.** Not a routine bump: the `opennms-36` dist serves one point release at a time, so 36.0.3 stopped having an installation candidate and a fresh deploy failed at the package install with `no available installation candidate`. Every package the roles install was verified at `36.0.4-1` against the repository index rather than inferred (#178).
- **VictoriaMetrics 1.150.0 to 1.151.0** in `stub_victoriametrics` (#172).

### Removed
- **Breaking: `opennms_repo_major` is gone.** The dist in `/etc/apt/sources.list.d/opennms.list` now derives from the caller's `opennms_version`, so the APT source and the version pin resolve from one fact and cannot name different majors. Set `opennms_version` — for the host or for the whole stack — and the repository follows. If you overrode `opennms_repo_major` to point at a mirror whose layout differs, use `opennms_repo_dist`, which is unchanged and has always been the documented variable for that. Lands in a MINOR bump under the `v0.x` rule in `RELEASING.md`, and belongs under `### Breaking Changes` in the release notes (#170).

### Fixed
- **The APT source will no longer be left at the previous major when the OpenNMS release moves.** `opennms_icmp` includes `opennms_repositories` on its own account, to install `jicmp` and `jicmp6`, and it passed no major — so it resolved a default that nothing kept current and rewrote the source *after* the calling role had written the right one. The last writer in every `opennms_core` and `opennms_minion` play was therefore the wrong one. It is invisible today only because that default happens to equal the shipped release's major, and the next major is where it stops being invisible: Renovate moves every `opennms_version` carrying a `# renovate:` annotation and left `opennms_repo_major` behind, so the version PR would have been green and the break would have arrived at the first deployment after the merge. Measured by pinning a major the default does not name: the source was written at the previous dist while the preferences file pinned the requested release, and the install then failed with `no available installation candidate` on a fresh host, or passed silently on a host that already carried the packages (#170).

- **The collection's task tags were unreachable.** Every tag defined only inside a dynamically included file was dead, because a dynamic include is entered only when the include task itself matches the tag filter and none of these includes carried tags. `--tags kafka`, `firewall`, `database`, `timeseries`, `java`, `icmp`, `repository`, `timesync` and nine others selected nothing at all — and exited 0, which is the bad part: a no-op that looks like a successful deployment. Each dynamic include now carries the union of the tags used inside what it pulls in, at every level, since a tagged include holding an untagged one is still a dead end. Inner tasks keep being filtered by their own tags, measured rather than assumed, so the union widens what can be reached and never what runs. The credentials-check includes are now tagged `always`, matching what their inner tasks already claimed, and a `firwall` typo on two Core tasks is corrected (#175).
- **`opennms_sentinel` pins `JAVA_HOME`,** as Minion already did. Both run upstream's `container.init`, which calls `find-java.sh 17.0 21.9999` and takes the *first* JDK in range, so a host carrying both 17 and 21 is a coin toss that can hand Karaf a JDK other than the one the `openjdk` role installed. The pin buys determinism. Minion's comment claiming the range rejects JDK 21 and aborts startup was measured false on 36.0.4 and is corrected, because left standing it tells the next reader the pin is load-bearing for startup and would justify tasks that are not needed (#174).
